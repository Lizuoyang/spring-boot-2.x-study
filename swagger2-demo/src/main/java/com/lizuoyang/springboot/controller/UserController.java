...
public class UserController {
    ...
    public List<UserVO> list() {
        ...
        result.add(UserVO.builder().id(2).username("revees").build());
        result.add(UserVO.builder().id(3).username("conner").build());
        result.add(UserVO.builder().id(1).username("lizuoyang").build());
        // 返回列表
        return result;
    }
    ...
    public UserVO get(@RequestParam("id") Integer id) {
        // 查询并返回用户
        return UserVO.builder().id(id).username(UUID.randomUUID().toString()).build();
    }
